...
#### Hypothesis 1
kfs_total_2009_2019 = DataFrame(XLSX.readtable("KFS_merge_data/KFS_2009_2019.xlsx", "Sheet1"))
...
did_cross_data_1_2 = dropmissing(did_cross_data_1_2)
did_cross_data_1_2 = filter(row -> isfinite(row[:매출액증가율tp1]) && isfinite(row[:상시근로자증가율]), did_cross_data_1_2)
...
did_cross_data_1_2_panel_anal_p = combine(groupby(did_cross_data_1_2, [:패널키, :기준연도]), nrow => :n)
did_cross_data_1_2_panel_anal_p = combine(nrow => :n, did_cross_data_1_2, :상시근로자기준dummy => mean => :상시근로자기준dummy_mean)
did_cross_data_1_2_panel_anal_p = select!(did_cross_data_1_2_panel_anal_p, Not([:n]))

# Pooled regression
ols_cross_3_1 = lm(@formula(상시근로자증가율 ~ 1 + 상시근로자기준dummy*Post + ln상시근로자수t + 매출액증가율tp1 + 매출액증가율t + 마진율t + 당좌비율t + 부채비율t + $result_indu), did_cross_data_1_2_panel_anal_p)
println(ols_cross_3_1)

# Regression with unit effect
did_cross_3_2 = reg(상시근로자증가율 ~ 상시근로자기준dummy*Post + ln상시근로자수t + 매출액증가율tp1 + 매출액증가율t + 마진율t + 당좌비율t + 부채비율t + EntityEffects + $result_indu, did_cross_data_1_2_panel_anal_p, fe=true, clustentity=true)
println(did_cross_3_2)

# Regression with time effect
did_cross_3_3 = reg(상시근로자증가율 ~ 상시근로자기준dummy*Post + ln상시근로자수t + 매출액증가율tp1 + 매출액증가율t + 마진율t + 당좌비율t + 부채비율t + TimeEffects + $result_indu, did_cross_data_1_2_panel_anal_p, fe=true, clusttime=true)
println(did_cross_3_3)

# Regression with two way effect
did_cross_3_4 = reg(상시근로자증가율 ~ 상시근로자기준dummy*Post + ln상시근로자수t + 매출액증가율tp1 + 매출액증가율t + 마진율t + 당좌비율t + 부채비율t + TimeEffects + EntityEffects + $result_indu, did_cross_data_1_2_panel_anal_p, fe=true, clustentity=true, clusttime=true)
...
kfs_panel_anal_did_2010_data_p = combine(groupby(kfs_panel_anal_did_2010_data, [:패널키, :기준연도]), nrow => :n)
kfs_panel_anal_did_2010_data_p = combine(nrow => :n, kfs_panel_anal_did_2010_data, :treat => mean => :treat_mean)
kfs_panel_anal_did_2010_data_p = select!(kfs_panel_anal_did_2010_data_p, Not([:n]))

# Pooled OLS regression
model_1 = reg(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, kfs_panel_anal_did_2010_data_p)
println(model_1)

# Panel OLS regression with unit effects
model_2 = reg(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, kfs_panel_anal_did_2010_data_p, fe=true, clustentity=true)
println(model_2)

# Panel OLS regression with time effects
model_3 = reg(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, kfs_panel_anal_did_2010_data_p, fe=true, clusttime=true)
println(model_3)

# Panel OLS regression with both entity and time effects
model_4 = reg(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, kfs_panel_anal_did_2010_data_p, fe=true, clustentity=true, clusttime=true)
println(model_4)

# Panel OLS regression with clustered standard errors by entity
model_year = reg(매출액증가율 ~ treat*yeardummy1 + treat*yeardummy2 + treat*yeardummy4 + treat*yeardummy5 + treat*yeardummy6 + treat*yeardummy7 + treat*yeardummy8, kfs_panel_anal_did_2010_data_p, fe=true, clustentity=true)
println(model_year)
