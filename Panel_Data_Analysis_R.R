...
data2 = data%>%filter(자산t<500000L & 자본총계t<100000L &
                        평균매출액기준tp1<80000L & 상시근로자수t<300L & 산업분류_대t == '제조업(10~33)' & 기준연도<2019
) 

data2=data2%>%mutate(d=log(매출액tp1)-log(매출액t), 마진율t = 법인세차감전순이익t/매출액t, 당좌비율t=당좌자산t/자산t, 부채비율t=부채t/자산t,매출액증가율t=log(매출액t)-log(매출액tm1))

data2=data2%>%filter(d!=Inf& d!=-Inf & 평균매출액기준tp1!='NA' & GR_EMPtp1!='NA' &GR_EMPtp1!=Inf & GR_EMPtp1!=-Inf &당좌비율t !=Inf & 부채비율t !=Inf)
post=if_else(data2$기준연도>=2014L,1,0)
treat=if_else(data2$상시근로자수t>=270L,1,0)
...
stargazer(mod1,mod2,mod3,mod4, omit=c('year','산업분류_중t','d','마진율t','매출액증가율t','당좌비율t','부채비율t'), type = 'html', out = 'Did비교2.doc',
...
model_2 <- plm(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, data = kfs_panel_anal_did_2010_data_p, model = "within", effect = "individual")
summary(model_2)

# Panel OLS regression with time effects
model_3 <- plm(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, data = kfs_panel_anal_did_2010_data_p, model = "within", effect = "time")
summary(model_3)

# Panel OLS regression with both unit and time effects
model_4 <- plm(매출액증가율 ~ treat*Post + ln매출액 + 마진율 + 부채비율, data = kfs_panel_anal_did_2010_data_p, model = "within", effect = "twoways")
summary(model_4)
